...
const RegistrationForm = ({ onSuccess, isAdminMode = false, onClose = null }) => {
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => {
      ...
      // Auto-generate preferred names when first or last name changes
      if (name === 'scoutFirstName' || name === 'scoutLastName') {
        const firstName = name === 'scoutFirstName' ? value : prev.scoutFirstName;
        const lastName = name === 'scoutLastName' ? value : prev.scoutLastName;
        // Only auto-generate if preferred name is empty or matches the previous auto-generated value
        const currentPreferred = prev.scoutPreferredName;
        const previousAutoGenerated = `${prev.scoutFirstName} ${prev.scoutLastName}`.trim();
        
        if (!currentPreferred || currentPreferred === previousAutoGenerated) {
          newData.scoutPreferredName = `${firstName} ${lastName}`.trim();
        }
      }

      if (name === 'fatherFirstName' || name === 'fatherLastName') {
        const firstName = name === 'fatherFirstName' ? value : prev.fatherFirstName;
        const lastName = name === 'fatherLastName' ? value : prev.fatherLastName;
        const currentPreferred = prev.fatherPreferredName;
        const previousAutoGenerated = `${prev.fatherFirstName} ${prev.fatherLastName}`.trim();
        
        if (!currentPreferred || currentPreferred === previousAutoGenerated) {
          newData.fatherPreferredName = `${firstName} ${lastName}`.trim();
        }
      }

      if (name === 'motherFirstName' || name === 'motherLastName') {
        const firstName = name === 'motherFirstName' ? value : prev.motherFirstName;
        const lastName = name === 'motherLastName' ? value : prev.motherLastName;
        const currentPreferred = prev.motherPreferredName;
        const previousAutoGenerated = `${prev.motherFirstName} ${prev.motherLastName}`.trim();
        
        if (!currentPreferred || currentPreferred === previousAutoGenerated) {
          newData.motherPreferredName = `${firstName} ${lastName}`.trim();
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isAdminMode) {
        // Admin mode: Create users directly with approved status
        console.log('🔧 Admin mode: Creating users with approved status...');
        const result = await authService.createUsersFromRegistration(submissionData);
        console.log('Users created:', result);
        
        // Update all created users to approved status
        for (const user of result.usersCreated) {
          await authService.updateUserProfile(user.id, { accessStatus: 'approved' });
        }
        
        console.log('✅ Family successfully registered and approved!');
      } else {
        // Normal mode: Create users with pending status (awaiting admin approval)
        const result = await authService.createUsersFromRegistration(submissionData);
        ...
        <Card sx={{ mb: 3, backgroundColor: '#e3f2fd' }}>
          <CardContent>
            ...
            <Grid container spacing={2}>
              ...
              {/* Second Row: Preferred Name (full width) */}
              <Grid item xs={12}>
                <TextField
                  fullWidth
                  ...
                  onChange={handleChange}
                  disabled={loading}
                  helperText="Auto-generated from first and last name, but you can customize it"
                />
              </Grid>
